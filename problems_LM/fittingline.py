...
pt = [Vec(v) for v in np.random.rand(10,2)]

def verticaldist(point,a,b):
    '''
    pt: 2D array of form np.array([x,y])
    a: gradient of line
    b: intercept with y-axis
    Function returns vertical distance between point and line.
    '''
...
li = [Vec(1),Vec(1)]
for i in pt:
    u.add_residual(partial(verticaldist,i),li[0],li[1])
for f,args,pranges in u.residuals:
    res,pders = Jet.compute_first_order(f,*args)
...
u = Optimizer()
for x,y in pts:
    u.add_residual(partial(vert_dist,x,y),li[0],li[1])
